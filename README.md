# Factory Clicker

An incremental factory automation game built with Flutter. Gather raw
materials, divide machines between production lines, research new technology,
and launch rockets for permanent Space Science bonuses.

[![Web build and deploy](https://github.com/Tranquility2/factory-clicker/actions/workflows/deploy-web.yml/badge.svg?branch=main)](https://github.com/Tranquility2/factory-clicker/actions/workflows/deploy-web.yml)
[![Web release version](https://img.shields.io/github/v/release/Tranquility2/factory-clicker?label=version)](https://github.com/Tranquility2/factory-clicker/releases/latest)
[![Play in your browser](https://img.shields.io/badge/Play-GitHub%20Pages-2ea44f?logo=github)](https://tranquility2.github.io/factory-clicker/)
[![Built with Flutter](https://img.shields.io/badge/Built%20with-Flutter-02569B?logo=flutter&logoColor=white)](https://flutter.dev/)
[![AI assisted by GitHub Copilot](https://img.shields.io/badge/AI%20assisted-GitHub%20Copilot-000000?logo=githubcopilot&logoColor=white)](https://github.com/features/copilot)

![Factory Clicker late-game factory](factory-late-game-full.png)

## Highlights

- Concurrent mining, smelting, and assembly lines
- Animated factory topology with live material flow
- Technology progression and rocket prestige
- Offline progress and portable local saves
- Web, Linux, and Windows targets

## Documentation

- [Game overview](docs/game-overview.md)
- [Development guide](docs/development.md)
- [Architecture](docs/architecture.md)
- [Technical reference](docs/reference.md)
